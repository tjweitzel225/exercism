...
pub fn solve(
    ...
    loop {
        ...
        match state {
            ...
            // Skip to solution if filling b would get you there
            ...
            (a, b) if a == goal || b == goal => {
                let (goal_bucket, other_bucket) = if b == goal {
                    (Bucket::Two, a)
                } else {
                    (Bucket::One, b)
                };
            ...
            (a, b) if a + b <= capacity_2 => state = (0, a + b),
            // Left doesn't fit in right, we only pour remainder
            (a, b) => state = (a + b - capacity_2, capacity_2),
        }
    }
}
